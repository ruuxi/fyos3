  ...
  const result = streamText({
    ...
    system:
      [
        ...
        'Workspace map: src/app (Next.js routes/APIs like api/agent/route.ts), src/components (UI like WebContainer.tsx, AIAgentBar.tsx), src/apps/<id>/index.tsx (desktop apps), public/apps/registry.json (app registry), src/utils (WebContainer/runtime helpers), src/lib (utilities), src/data (static data), templates/webcontainer/src/desktop/Desktop.tsx (desktop shell).',
        ...
        'Tool usage policy: prefer direct file targets using the workspace map over calling web_fs_find; for multi-file operations, use batch tools (web_fs_read_many, web_fs_write_many); when writing, skip if content is unchanged.',
