...
export type ChatTabsProps = {
  ...
  onOpenFromHistory: (id: string) => void;
};

export default function ChatTabs(props: ChatTabsProps) {
  const { openThreads, historyThreads, threadsLoading, threadsError, activeThreadId, setActiveThreadId, showHistory, setShowHistory, onRefresh, onNewConversation, onClose, onOpenFromHistory } = props;
  return (
    <div className="mb-2">
      ...
      {showHistory && (
        <div className="mt-2 max-h-[220px] overflow-auto rounded border border-white/20 bg-white/10 p-2">
          {historyThreads.length === 0 && (
            <div className="text-xs text-white/60 px-1 py-2">No saved chats yet.</div>
          )}
          {historyThreads.map((t) => (
            <button
              key={t._id}
              onClick={() => {
                onOpenFromHistory(t._id);
                setShowHistory(() => false);
              }}
              className={`w-full text-left text-xs rounded px-2 py-1 ${activeThreadId === t._id ? 'bg-white/20 text-white' : 'text-white/80 hover:bg-white/15'}`}
            >
